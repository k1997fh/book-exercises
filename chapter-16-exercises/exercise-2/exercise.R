...
ggplot(data = diamonds) + geom_col(mapping = aes(x = cut, y = price, filled = clarity))

# Draw the same chart again, but with each element positioned to "fill" the y axis
ggplot(data = diamonds) + geom_col(mapping = aes(x = cut, y = price, filled = clarity), position = "fill")

# Draw the same chart again, but with each element positioned to "dodge" each other
ggplot(data = diamonds) + geom_col(mapping = aes(x = cut, y = price, filled = clarity), position = "dodge")
...
ggplot(data = diamonds) + geom_point(mapping = aes(x = cut, y = clarity)) + scale_y_log10()

# For another version, draw the same plot but with `violin` geometry instead of 
# `boxplot` geometry!
# How does the logarithmic scale change the data presentation?
ggplot(data = diamonds) + geom_violin(mapping = aes(x = cut, y = clarity))
...
ggplot(data = diamonds) + geom_col(mapping = aes(x = carat, y = price, position))
